...
#    -> 当 N=120, Batch=16 时: 1.5 + 16 * 1 * 0.3 = 6.3GB (吃满 7G 可用显存)
...
# ===============================================================================
num_samples = 300  # 每次运行 generator.py 生成的样本总数。深度学习中，数据量越大，最终 MAE 误差越低。
...
# ===============================================================================
batch_size = 4  # 每次喂给U-Net的图像数量。若报OOM（Out of Memory），请果断更改为当前使用的最大显存。
epochs = 30  # 整个样本的数据集将被学习X遍。
...
def build_custom_structure(is_batch_generation=False) -> Region:
    ...
    #return my_structure
        cx, cy, cz = 60, 60, 60
        r = 10
        return SphereRegion(center=(cx, cy, cz), radius=r)
